...
import { RenderTokenizedText } from './TokenHighlight'
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="px-6 py-3 bg-white border-b border-gray-200 flex items-center gap-2 shrink-0">
        ...
        <span className="ml-auto text-xs text-gray-400">
          {viewMode === 'user' ? 'Showing rehydrated conversation' : 'Hover tokens to inspect · Click Edit to remap · Select text to tag'}
        </span>
      </div>
...
function MessageBubble({ message, viewMode, identityChart, dictionaries, onManualTag, onMapToExisting }) {
  const isUser = message.role === 'user'
  const isLlmView = viewMode === 'llm'
  ...
  return (
    <div className={`flex gap-3 ${isUser ? 'flex-row-reverse' : ''}`}>
      ...
      <div className={`max-w-[70%] rounded-xl px-4 py-3 text-sm leading-relaxed ${
        ...
        {isLlmView ? (
          // LLM View: render with interactive token chips (hover + edit)
          <RenderTokenizedText
          ...
          <p className="whitespace-pre-wrap">{message.hydratedText}</p>
        )}
        {isLlmView && message.detections?.length > 0 && (
